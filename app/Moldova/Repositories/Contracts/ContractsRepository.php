...
class ContractsRepository implements ContractsRepositoryInterface
{
     ...
     * {@inheritdoc}
     */
    public function getContractsBySource()
    {
        $result = OcdsRelease::raw(
            function ($collection) {
                return $collection->aggregate(
...
//        array_push($query, $filter);
    ...
    {
	//$result = $this->ocdsRelease->where('contracts.id', $contractId)->orWhere('contracts.id', '=', (int) $contractId)->project(['contracts.$' => 1, 'awards' => 1, 'tender.id' => 1, 'tender.title' => 1, 'buyer.name' => 1])->first();
        $result = $this->ocdsRelease->whereIn('contracts.id', [$contractId, (int) $contractId])->project(['contracts.$' => 1, 'awards' => 1, 'tender.id' => 1, 'tender.title' => 1, 'buyer.name' => 1])->first();
...
//                return $collection->find(['contracts.id' => $contractId]);
		return $collection->find(['contracts.id' => ['$in' => [(int) $contractId, $contractId]]]);
            }
        );
...
//        return $this->ocdsRelease->where('contracts.id', (int) $contractId)->first();
    ...
    public function getDistinctContractorsEtenderCount()
    {

        $result = CompaniesEtenders::raw(
            function ($collection) {
                return $collection->distinct('identifierId');
            }
        );
