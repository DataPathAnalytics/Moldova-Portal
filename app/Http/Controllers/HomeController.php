...
class HomeController extends Controller
{
	...
	$sourceDistinct      = $this->tenders->getDistinctSource();
        $tendersBySource     = $this->tenders->getTendersBySource($sourceDistinct);
        $contractsBySource   = $this->contracts->getContractsBySource($sourceDistinct);
        $trendsBySource      = $this->prepareTrendsBySource($tendersBySource, $contractsBySource);
        ...
        return view('index', compact('totalContractAmount', 'trends', 'procuringAgency', 'contractors', 'goodsAndServices', 'contractTitles', 'procuringAgencies', 'trendsBySource'));
    }
    ...
    protected function prepareTrendsBySource($tendersBySource, $contractsBySource)
    {
        $preparedTrends = [];

        foreach ($tendersBySource as $key => $value) {
            array_push($preparedTrends, [
                'year' => $key,
                'data' => [
                    'tenders' =>  $value,
                    'contracts' => $contractsBySource[$key]
                ]
            ]);
        }

        return json_encode($preparedTrends);
    }
